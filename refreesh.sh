...
refresh_one_url() {
    ...
    if [ "$task_type" = "$preget" -o "$task_type" = "$purge" ]; then
        if [ "$device_type" = n ]; then
            text=$(nginx_purge "$device_ip" "$url")
        else
            text=$(ats_purge "$device_ip" "$url")
        fi
        test $? -ne 0 && res=Failed
    fi
    if [ "$task_type" = "$preget" ]; then
        text=$(get "$device_ip" "$url")
        test $? -ne 0 && res=Failed
    fi
...
ats_purge() {
    ...
    ip=$1
    url=$2
    curl --retry 3 --retry-delay 10 -k -m "$curl_tmout" "http://$ip/delete_url?url=$url"
}
...
nginx_purge() {
    ...
    ip=$1
    url=$2
    curl --retry 3 --retry-delay 10 -k -m "$curl_tmout" -I $url -x $ip:80 -H "X-MPROXY-PURGE:1"
}
...
get() {
    ...
    ip=$1
    url=$2
    curl --retry 3 --retry-delay 10 -k -m "$curl_tmout" -x ${ip}:80 ${url} -I -X HEAD
}
